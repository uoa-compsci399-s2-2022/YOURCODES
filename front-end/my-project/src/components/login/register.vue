<template>
  <div>
    <header>
      ...
      <el-image class="user" :src="require('../../assets/Images/user.png')" @click="func4" width="30px" height="30px"></el-image>
      <hr>
      <nav>
        <div class="navbar">
    ...
    <div>
      <div class="login_form">
        ...
        <br>
        <!-- username -->
        <input type="email" name="" v-model="dataForm.email" id="email" placeholder="Email" required>
        <br>
        <!-- username -->
        <input v-model="dataForm.username" type="text"  id="username" placeholder="Username" required>
        <br>
        <!-- password -->
        <input v-model="dataForm.password" type="password"  id="password" required placeholder="Password">
        <br>
        <!-- submit button -->
...
<script>
  export default {
    ...
    func4() {
      ...
      // The form submission
      dataFormSubmit () {
           if(this.dataForm.username === ''||this.dataForm.email === ''||this.dataForm.password === ''){
             this.$message.error('Please enter the complete information')
             return
           }
            this.axios({
              url: `http://localhost:8080/login/user/save`,
              method: 'post',
              data: this.dataForm
            }).then(({data}) => {
